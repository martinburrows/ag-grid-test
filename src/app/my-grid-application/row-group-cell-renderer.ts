...
import {ExpressionService} from "ag-grid/main";
import {EventService} from "ag-grid/main";
import {Constants} from "ag-grid/main";
import {Utils as _} from "ag-grid/main";
import {Autowired, Context} from "ag-grid/main";
import {Component} from "ag-grid/main";
import {ICellRenderer, ICellRendererParams} from "ag-grid/main";
import {RowNode} from "ag-grid/main";
import {CellRendererService} from "ag-grid/main";
import {ValueFormatterService} from "ag-grid/main";
import {CheckboxSelectionComponent} from "ag-grid/main";
import {ColumnController} from "ag-grid/main";
import {Column} from "ag-grid/main";
import {RefSelector} from "ag-grid/main";
import { GroupCellRendererParams } from "ag-grid/dist/lib/rendering/cellRenderers/groupCellRenderer";

let svgFactory = SvgFactory.getInstance();

export class RowGroupCellRenderer extends Component implements ICellRenderer {

    private static TEMPLATE =
        '<span class="ag-group-cell-wrapper" ref="eGroupCellWrapper">' +
    ...
    constructor() {
        super(RowGroupCellRenderer.TEMPLATE);
        this.element = super.getGui();
    }
    ...
    private setupIndentation() { 
        let level = this.params.data.level;
        
        let indentation = 15 * level;

        if (indentation) { 
            _.addStylesToElement(this.element, {'margin-left':`${indentation}px`});
        }
    }
    ...
    private showExpandAndContractIcons(): void {
        ...
        if (rowNode.data.hasChildren) { 
            let expanded = rowNode.data.expanded;
...
/*
        ...
        console.log('showExpandAndContractIcons', expandable, rowNode);
        if (expandable) {
            // if expandable, show one based on expand state.
            // if we were dragged down, means our parent is always expanded
            let expanded = this.draggedFromHideOpenParents ? true : rowNode.expanded || rowNode.data.expanded;
            _.setVisible(this.eContracted, !expanded);
            _.setVisible(this.eExpanded, expanded);
        } else {
